...
import com.nicebin.common.utils.Tool;
import com.springclouddemo.redis.cache_interface.I_SystemCacheMgr;
...
public class RedisCacheMgr extends RedisCacheManager implements I_SystemCacheMgr {
    ...
    //系统缓存过期时间允许最小值（秒）
    private long minExpire = 0;
    //系统缓存过期时间允许最大值（秒）
    private long maxExpire = 0;

    private DefaultListableBeanFactory defaultListableBeanFactory;

    public RedisCacheMgr(RedisCacheWriter cacheWriter, RedisCacheConfiguration defaultCacheConfiguration, Map<String, RedisCacheConfiguration> initialCacheConfigurations, boolean allowInFlightCacheCreation,ApplicationContext applicationContext,long minExpire,long maxExpire) {
    ...
    public boolean isApproachExpire(String cacheName, Object id, Timestamp saveTime) throws NoSuchAlgorithmException {
        ...
        if (ttl != -1 && saveTime!=null) {
                int random = Tool.getSecureRandom(minExpire, maxExpire);
                Date date = new Date();
