...
import com.springclouddemo.redis.annotation.UpdateCache;
import com.springclouddemo.redis.cache_interface.I_SystemCacheMgr;
import com.springclouddemo.redis.task.UpdateCacheTask;
import lombok.Data;
...
public class SystemCacheMgr {
    ...
    //系统的线程池类
    @Autowired
    private CacheThreadPool cacheThreadPool;
    //所有缓存的所有数据记录Map
    ...
    public void autoUpdate(String cacheName, Object id) throws Exception {
        ...
        if (defaultCacheMgr.isApproachExpire(cacheName, id, dataInfo.saveTime)) {
            if (dataInfo.lock.tryLock()) { //这里用非阻塞的tryLock，有一个用户进来触发缓存过期判断即可，其他用户继续获得当时的缓存值
                //获取锁后再次判断数据是否过期
                if (defaultCacheMgr.isApproachExpire(cacheName, id, dataInfo.saveTime)) {
                    ThreadPoolExecutor threadPoolExecutor = cacheThreadPool.getThreadPoolExecutor();
